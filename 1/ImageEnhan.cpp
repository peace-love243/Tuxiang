...
ImageEnhan::ImageEnhan(CSize size, int nBitCount, LPRGBQUAD lpColorTable, unsigned char *pImgData) :
...
* 说明： 在view中图像添加椒盐噪声
**********************************************************************/
void ImageEnhan::AddPepperSaltNoise()
{
	unsigned char* pDIB;
	int bytecount, i;
	if (m_pImgDataOut != NULL)
...
void ImageEnhan::ChooseMaskSmooth()
{
	if (m_nBitCount != 8)
	{
		AfxMessageBox("只能处理8位灰度图像!");
...
void ImageEnhan::GradeSharp(int Thresh)
{
...
void ImageEnhan::NeiAveTemplate(int TempH, int TempW, int TempCX,
...
void ImageEnhan::MedianSmooth(int FilterH, int FilterW, int FilterCX, int FilterCY)
{
	unsigned char*	pSrc;
	unsigned char*	pDst;
	int		i, j, k, l;
...
unsigned char ImageEnhan::FindMedianValue(unsigned char* pbArray, int ArrayLen)
{
	int i, j;
	unsigned char Temp;
...
void ImageEnhan::LapTemplate(int TempH, int TempW, int TempCX, int TempCY, float *fpTempArray, float f)
{
	unsigned char*	pSrc;
	unsigned char*	pDst;
	int		i, j, k, l;
